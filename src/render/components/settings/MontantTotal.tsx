  ...
  return (
    <SettingsWrapper >
      {useMemo(() => <Header areas={"'code code format direction . add'"}  >
        ...
        <AddIconSettings className='add-btn' onClick={() => Settings.genMontantTotal()} />
      </Header>, [filters])}
      <Body >
        {
          MontantTotal.filter(s => filter(filters, s)).map((s, i) => (<ItemSetting id={s.id} key={i} rm={remove} em={em} format={s.format} code={s.code} active={s.active} direction={s.direction} />))
        }
      </Body>
    </SettingsWrapper>
  )
