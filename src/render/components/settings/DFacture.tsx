...
const DFacture: FC = observer(({ }: Props) => {
    const { Settings } = usePersistentStore()
    let { DFacture } = Settings
    let emtyfilter = { code: "", format: "", direction: "" }
    const [filters, setFilters] = useState({ code: "", format: "MMYYYY", direction: "" });
    const remove = (id: string) => {
        Settings.rmDFacture(id)
    }
    const em = (id: string, payload: any) => {
        Settings.editDFacture(id, payload)
    }
    return (
        <SettingsWrapper >
            {useMemo(() => <Header areas={"'code code format direction . add'"}  >
                ...
                <AddIconSettings className='add-btn' onClick={() => Settings.genDFacture()} />
            </Header>, [filters])}
            <Body >
                {
                    DFacture.filter(s => filter(filters, s)).map((s, i) => (<ItemSetting id={s.id} key={i} rm={remove} em={em} format={s.format} code={s.code} active={s.active} direction={s.direction} />))
                }
            </Body>
        </SettingsWrapper>
    )
...
const ItemSetting = ({ em, id, rm, code, format, direction, active }: itemsSetting) => {
    ...
    const DIRECTIONS = ["MMYYYY","DDYYYY","DDMM","DDMMYYYY", "MMDDYYYY", "MMYYYYDD"]
    const { Option } = Select;
    ...
    return (
      ...
      setTemp({...item, code:ev.currentTarget.value.toLocaleUpperCase(),})
            ...
            <ItemDate size='small' picker="month" ref={inputdate as any}  style={{ width: '80%' }} area='code' format={item.format} onChange={(date: any, dateString: string) => {
                setITEM({ ...item, code: moment(dateString, item.format).toString(), active: false })
                setTemp({ ...item, code: moment(dateString, item.format).toString(), })
            }} />
