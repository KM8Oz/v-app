...
const Quantity: FC = observer(({ }: Props) => {
  ...
  const remove = (id: string) => {
    Settings.rmQuantity(id)
  }
  const em = (id: string, payload: any) => {
    Settings.editQuantity(id, payload)
  }
  return (
    <SettingsWrapper >
      {useMemo(() => <Header areas={"'code code format direction . add'"}  >
        ...
        <AddBtnSettings children={"add"}  onClick={() => Settings.genQuantity()} />
      </Header>, [filters])}
      <Body >
        {
          Quantity.filter(s => filter(filters, s)).map((s, i) => (<ItemSetting id={s.id} key={i} rm={remove} em={em} format={s.format} code={s.code} active={s.active} direction={s.direction} />))
        }
      </Body>
    </SettingsWrapper>
  )
