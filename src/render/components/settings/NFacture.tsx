  ...
  return (
    <SettingsWrapper >
      {useMemo(() => <Header areas={"'code code format direction . add'"}  >
        ...
        <AddIconSettings className='add-btn' onClick={() => Settings.genNFacture()} />
      </Header>, [filters])}
      <Body >
        {
          NFacture.filter(s => filter(filters, s)).map((s, i) => (<ItemSetting id={s.id} key={i} rm={remove} em={em} format={s.format} code={s.code} active={s.active} direction={s.direction} />))
        }
      </Body>
    </SettingsWrapper>
  )
  ...
  return (
    <SettingItem areas={"'code code format direction remove add'"} >
      ...
      <ItemCode area='code' type='text' onClick={()=> setITEM({ ...item, code:"", active: false })}  onChange={(ev) => {
        ...
        setITEM({ ...item, code: ev.currentTarget.value.match(/[0-9]{0,8}/g)[0], active: false })
        setTemp({ ...item, code: ev.currentTarget.value.match(/[0-9]{0,8}/g)[0] })
