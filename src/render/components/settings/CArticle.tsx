    ...
    return (
        <SettingsWrapper >
            {useMemo(()=><Header  >
                ...
                <IconAdd onClick={()=>Settings.genCArticle()} />
            </Header>,[filters])}
            <Body >
              {
                CArticle.filter(s=>filter(filters, s)).map((s, i) => (<ItemSetting id={s.id} key={i} rm={remove} em={em} nom={s.nom} format={s.format} code={s.code} active={s.active} direction={s.direction}  />))
              }
            </Body>
        </SettingsWrapper>
    )
