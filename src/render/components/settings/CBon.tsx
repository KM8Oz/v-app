...
const CBon:FC = observer(({ }: Props) => {
      const { Settings } = usePersistentStore()
      let { CBon } = Settings
      let emtyfilter = { letter:"", code:"", format:"", direction:""}
      const [filters, setFilters] = useState({letter:"", code:"", format:"", direction:""});
      const remove = (id:string)=>{
        Settings.rmCBon(id)
      }
      const em = (id:string,payload:any)=>{
        Settings.editCBon(id,payload)
      }
    return (
        <SettingsWrapper >
            {useMemo(()=><Header areas={"'nom code format direction . add'"}  >
                <Nomination type='text' maxLength={10} onChange={(ev=>setFilters({...emtyfilter, letter:ev.currentTarget.value.replace(/[^a-zA-Z]{1}/g, '')}))}  value={filters.letter} placeholder='Nomination' />
                ...
                <AddIconSettings className='add-btn' onClick={()=>Settings.genCBon()} />
            </Header>,[filters])}
            <Body >
              {
                CBon.filter(s=>filter(filters, s)).map((s, i) => (<ItemSetting letter={s.letter} id={s.id} key={i} rm={remove} em={em} format={s.format} code={s.code} active={s.active} direction={s.direction}  />))
              }
            </Body>
        </SettingsWrapper>
    )
...
const ItemSetting = ({em,id,rm, code, format, direction,letter, active}:itemsSetting)=>{
        const [item, setITEM] = useState({letter, code, format, direction, active});
        const [temp, setTemp] = useState({letter, code, format, direction, active});
   return (
    <SettingItem areas={"'nom code format direction remove add'"} >
    <ItemNom type='text' maxLength={10} onChange={(ev=>setITEM({...item, letter:ev.currentTarget.value.replace(/[^a-zA-Z]{1}/g, ''), active:false}))} placeholder='Nom' value={item.letter} />
    <ItemCode area='code' type='text' maxLength={8} onChange={(ev)=>{
    ...
    <ItemFormat type='text' maxLength={8} onChange={(ev=>setITEM({...item, format:ev.currentTarget.value.replace(/[^XYxy]/g, "").toLocaleUpperCase(), code:frmt(temp.code, ev.currentTarget.value.replace(/[^XY]/g, ""), item.active), active:false}))} placeholder='Format' value={item.format} />
