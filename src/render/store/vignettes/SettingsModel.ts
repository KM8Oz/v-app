...
const SettingsModel = types.model({
    ...
    .actions((self) => ({
        setismaster(v?:boolean){
            self.ismaster = !!v;
        },
        genCArticle(status?:boolean) {
        ...
        genCFournisseurs(status?:boolean) {
        ...
        genMontantTotal(status?:boolean) {
    ...
    .views((self) => ({
        init(){
            if(self.Station?.length == 0 && 
                self.CArticle?.length == 0 &&
                self.CBar?.length == 0 &&
                self.CBon?.length == 0 &&
                self.CFournisseurs?.length == 0 &&
                self.MontantTotal?.length == 0 &&
                self.NFacture?.length == 0 &&
                self.Station?.length == 0 &&
                self.DFacture?.length == 0){
                    self.genCArticle(true)
                    self.genCBar(true)
                    self.genCBon(true)
                    self.genCFournisseurs(true)
                    self.genDBon(true)
                    self.genDFacture(true)
                    self.genMontantTotal(true)
                    self.genPU(true)
                    self.genQuantity(true)
                    self.genStation(true)
                    self.genNFacture(true)
                    return "ok"
            }
        },
